...
export const reqAllUser = (page: number, limit: number, username: string) =>
  request.get<any, User>(API.ALLUSER_URL + `${page}/${limit}/?username=${username}`)
// 新增账号请求|更新账号信息请求
...
// 获取选中账户全部角色请求
export const reqGetAllRoles = (adminId: number) => request.get<any, Role>(API.ALLROLE_URL + adminId)

// 分配职位请求
export const reqAssignRole = (data: AssignRoleData) => request.post<any, any>(API.ASSIGNROLE_URL, data);

// 删除单个用户
export const reqDeleteSingleUser = (id: number) => request.delete<any, any>(API.DELETESINGLEUSER_URL + id);
