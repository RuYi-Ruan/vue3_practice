<template>
  <el-card style="margin: 10px 0;">
    <el-form :inline="true" class="form">
      <el-form-item label="角色名称">
        <el-input type="text" placeholder="请输入职位名称" v-model="roleName">
        </el-input>
      </el-form-item>

      <el-form-item>
        <el-button type="primary" @click="search" :disabled="roleName ? false : true">搜索</el-button>
  ...
  <el-card>
    <el-button type="primary" plain icon="Plus" @click="addRole">添加职位</el-button>
    <el-table border style="margin: 10px 0;" :data="roleArr">
      ...
      <el-table-column label="操作" align="center" width="320px">
        <template #="{row, $index}">
          <el-button type="primary" plain icon="User">分配权限</el-button>
          <el-button type="warning" plain icon="Edit" @click="updateRole(row)">编辑</el-button>
...
import { reqAddOrUpdateRole, reqGetAllRole } from '@/api/acl/role';
...
const getHasRole = async (pager=1) =>{
  pageNo.value=pager
  let result:RoleResponseData = await reqGetAllRole(pageNo.value, pageSize.value, roleName.value);
  if(result.code == 200){
