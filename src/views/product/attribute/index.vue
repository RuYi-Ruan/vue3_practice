<template>
  <div>
    ...
    <el-card style="margin: 10px 0">
      <div v-show="scene == 0">
        <el-button type="primary" size="default" icon="Plus" :disabled="getDisabled()" @click="addAttr()">
          添加属性
        </el-button>
        <el-table border style="margin: 10px 0" :data="attrArr">
          ...
          <el-table-column label="操作" width="120px" align="center">
            ...
            <template #="{ row, $index }">
              <el-button size="small" type="warning" icon="Edit" @click="updateAttr(row)"></el-button>
              <el-popconfirm :title="`确认删除${row.attrName}吗？`" width="250px" icon="QuestionFilled"
      ...
      <div v-show="scene == 1">
        ...
        <el-button size="default" type="info" icon="CircleClose" @click="cancel">
          取消
        </el-button>
        ...
        <el-table border style="margin: 10px 0" :data="attrParams.attrValueList">
          ...
          <el-table-column align="center" label="属性值" width="480px">
            <!-- row即为当前属性值对象 -->
            <template #="{ row, $index }">
              <el-input placeholder="请你输入属性值名称" v-model="row.valueName" v-if="row.flag" @blur="toLook(row, $index)"
                :ref="(vc: any) => inputArr[$index] = vc">
              </el-input>
              <div v-else @click="toEdit(row, $index)" role="button">{{ row.valueName }}</div>
            </template>
          </el-table-column>
          <!-- 操作 -->
          <el-table-column label="操作" width="120px" align="cenyer">
            <template #="{ row, $index }">
              <el-button type="danger" size="default" icon="Delete"
                @click="attrParams.attrValueList.splice($index, 1);">Delete</el-button>
            </template>
          </el-table-column>
        </el-table>

        <el-button type="primary" size="default" icon="CircleCheck" @click="save"
          :disabled="attrParams.attrValueList.length === 0 || attrParams.attrValueList.some(item => item.valueName === '')">
          保存
        </el-button>
        <el-button size="default" type="info" icon="CircleClose" @click="cancel">
          取消
...
// 引入获取已有属性与属性值
import { reqAttr, reqAddOrUpdateAttr, reqRemoveAttr } from '@/api/product/attribute'
...
const save = async () => {
  // 校验属性值列表，确保没有空值
  const hasEmptyValue = attrParams.attrValueList.some(item => item.valueName === '');
