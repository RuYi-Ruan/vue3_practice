<template>
  <el-form label-width="100px">
    ...
    <el-form-item label="SPU销售属性">
      <!-- 下拉菜单，选择销售属性 -->
      <el-select :placeholder="unSelectSaleAttr.length ? `剩余${unSelectSaleAttr.length}项未选` : '暂无数据可选'"
      ...
      <el-table border style="margin: 10px 0" :data="sale">
          ...
          <template #="{ row, $index }">
            <div>
              ...
              <el-input v-model="row.saleAttrValue" v-if="row.flag == true" placeholder="请输入属性值" size="small"
                style="width: 100px;" @blur="toLook(row)">
              </el-input>
        ...
        <el-table-column label="操作" align="center" width="120px">
          <template #="{ row, $index }">
            <el-button type="danger" size="small" icon="Delete" title="删除SKU" @click="sale.splice($index, 1)">
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
const addSaleAttr = () => {
  ...
  allSale.value = allSale.value.filter(item => item.id.toString() !== baseSaleAttrId);
